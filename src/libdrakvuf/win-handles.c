...
{
    bool ret        = false ;
    addr_t obj_header_addr = 0 ;

    obj_header_addr = drakvuf_get_obj_by_handle( drakvuf, current_eprocess, handle );
    ...
    if ( obj_header_addr )
    {
        ...
        ctx.addr = obj_header_addr + drakvuf->offsets[ OBJECT_HEADER_TYPEINDEX ] ;

        if (!drakvuf->osi.get_object_type_index(drakvuf, &ctx, &object_type_index))
        {
            return false;
        }

        if ( object_type_index == obj_type_arg )
        {
            if ( object_type_index == OBJ_MANAGER_PROCESS_OBJECT )
            {
                // Object Body must be an _EPROCESS...
                ret = drakvuf_is_process( drakvuf, info->regs->cr3, obj_header_addr + drakvuf->offsets[ OBJECT_HEADER_BODY ] );
            }
            else if ( object_type_index == OBJ_MANAGER_THREAD_OBJECT )
            {
                // Object Body must be an _ETHREAD...
                ret = drakvuf_is_thread( drakvuf, info->regs->cr3, obj_header_addr + drakvuf->offsets[ OBJECT_HEADER_BODY ] );
            }
    ...
    if ( ret )
    {
        *obj_body_addr = obj_header_addr + drakvuf->offsets[ OBJECT_HEADER_BODY ];
    }
